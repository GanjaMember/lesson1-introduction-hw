...
# 1) При разных запусках наибольшее ускорение на GPU получают матричное умножение и поэлементное умножение
# Это самые тяжело вычисляемые операции, а, например, для сравнения операция сложения в разы проще них
# 2) Потому что на GPU для параллельного выполнения операция они разбиваются на кернелы, которые
# имеют фиксированное время выполнения, например, 2 - 50 мс. Если CPU выполняет эти операции быстрее времени
# кернелов, то операции на CPU выполняться раньше
# 3) При увеличении размера производительность на CPU падает, а на GPU возрастает, так как на GPU вычисления
# идут параллельно
# 4) При передаче данных между CPU и GPU выделяется память на видеокарте (VRAM), затем данные копируются из 
# оперативной памяти компьютера (RAM) на VRAM через шину PCIe. Эта операция инициируется специальными функциями
# и может проходить синхронно или асинхронно, при этом скорость передачи ограничивается пропускной способностью PCIe